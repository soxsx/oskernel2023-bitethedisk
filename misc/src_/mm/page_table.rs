...
impl PageTable {
    ...
    pub fn remap_cow(&mut self, vpn: VirtPageNum, ppn: PhysPageNum, former_ppn: PhysPageNum) {
        ...
        *pte = PageTableEntry::new(ppn, pte.flags() | PTEFlags::W);
        pte.set_cow();
        ppn.as_bytes_mut().copy_from_slice(former_ppn.as_bytes_mut());
    }
}
...
pub fn translated_str(token: usize, ptr: *const u8) -> String {
    ...
    loop {
        let ch: u8 = *(page_table.translate_va(VirtAddr::from(va)).unwrap().as_mut());
        if ch == 0 {
            break;
        } else {
...
pub fn translated_ref<T>(token: usize, ptr: *const T) -> &'static T {
    ...
    let page_table = PageTable::from_token(token);
    page_table.translate_va(VirtAddr::from(ptr as usize)).unwrap().as_ref()
}
...
pub fn translated_refmut<T>(token: usize, ptr: *mut T) -> &'static mut T {
    ...
    let page_table = PageTable::from_token(token);
    let va = ptr as usize;
    page_table.translate_va(VirtAddr::from(va)).unwrap().as_mut()
}
