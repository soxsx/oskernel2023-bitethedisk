...
impl</*'a,*/ X: SPI> SDCard</*'a,*/ X> {
    ...
    fn get_dataresponse(&self) -> u8 {
        ...
        if response[0] != 0x05 {
            warn!("[get_dataresponse] Data no accepted !!! ");
            return 0xFF;
        }
        /* Wait null data */
        // self.read_data(response);
...
static PERIPHERALS: Mutex<Option<Peripherals>> = Mutex::new(None);
...
fn init_sdcard() -> SDCard<SPIImpl> {
    ...
    let mut sd = SDCard::new(SPIImpl { spi }, SD_CS, IS_HC);
...
impl SDCardWrapper {
    pub fn new() -> Self {
        Self(Arc::new(Mutex::new(init_sdcard())))
    }
}
